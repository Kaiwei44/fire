...
import numpy as np
import pandas as pd
from ...core.algorithm.newey_west_t_industrial import NeweyWestTTest
from ...core.eva_utils import factor_to_quantile, factor_to_quantile_dependent_double_sort
from ...core.eva_utils import _compute_quantile_df, _compute_weighted_quantile_df
from ...core.eva_utils import ForwardReturns, QuantileReturns
from ...core.eva_utils import double_sort_returns_independent
...
#2. Double sort based on return_adj
    ...
    def get_statistics(result: QuantileReturns, quantiles: int) -> StatisticResults:
        ...
        return StatisticResults({'t_stats': pd.DataFrame(t_stats, index=result.keys(), columns=period_returns.columns),
                                 'p_values': pd.DataFrame(p_values, index=result.keys(),
                                                          columns=period_returns.columns),
                                 'se_values': pd.DataFrame(se_values, index=result.keys(),
                                                           columns=period_returns.columns),
                                 'mean_returns': pd.DataFrame(mean_returns, index=result.keys(),
                                                              columns=period_returns.columns)})
